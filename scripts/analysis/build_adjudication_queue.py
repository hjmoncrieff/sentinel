#!/usr/bin/env python3
"""
Build a private adjudication queue for irregular-transition target review.

This stage turns the review artifact into a cleaner working queue so analysts
can expand adjudicated labels country by country without replacing the broader
proxy target layer.
"""
...
OUT = ROOT / "data" / "review" / "adjudication_queue_irregular_transition.json"
LOCAL_DECISIONS = ROOT / "data" / "review" / "adjudicated_transition_decisions.local.json"
...
def build() -> dict:
    ...
    for country_row in countries:
        ...
        for case in country_row.get("cases", []):
            ...
            if rating not in QUEUE_RATINGS:
                continue
            key = (country, str(case.get("trigger_month") or ""), "irregular_transition_next_1m")
    ...
    return {
        ...
        "status": "private_internal_adjudication_queue",
        ...
        "description": (
            "Working queue of irregular-transition proxy positives that are not "
            "yet promoted into the adjudicated label layer."
        ),
...
def main() -> None:
    ...
    print(f"Wrote adjudication queue to {OUT.relative_to(ROOT)}")
