#!/usr/bin/env python3
"""
Validate the broader acute political-risk target layer against the acute-risk
gold subset.

This is a private/internal checkpoint used before any future acute-risk model
fitting.
"""
...
OUT = ROOT / "data" / "review" / "gold_acute_political_risk_validation.json"
...
def main() -> None:
    ...
    for gold in gold_rows:
        ...
        if not panel_row:
            continue
        signal_score = int(panel_row.get("acute_political_risk_signal_score_next_1m") or 0)
        signal_label = str(panel_row.get("acute_political_risk_signal_label_next_1m") or "")
    ...
    predicted_positive_rows = [row for row in panel_rows if int(row.get("acute_political_risk_signal_score_next_1m") or 0) >= 4]
    ...
    output = {
        ...
        "status": "private_internal_gold_acute_political_risk_validation",
        "panel_file": str(PANEL.relative_to(ROOT)),
        "gold_file": str(GOLD.relative_to(ROOT)),
        "target_rule": panel_rows[0].get("acute_political_risk_target_rule") if panel_rows else None,
        "summary": {
    ...
    print(f"Wrote gold acute political-risk validation report to {OUT.relative_to(ROOT)}")
