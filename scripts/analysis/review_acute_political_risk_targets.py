#!/usr/bin/env python3
"""
Review proxy acute-political-risk positives country by country.

This runner is private/internal. It helps separate broad acute deterioration
signals into more interpretable categories before the project creates
adjudicated targets.
"""
...
OUT = ROOT / "data" / "review" / "acute_political_risk_target_review.json"
...
def classify_case(candidate: dict) -> tuple[str, str]:
    if candidate.get("event_type_coup_count", 0) > 0:
        return "strong", "next month contains a coup-coded acute deterioration signal"
    if (
        ...
        return "strong", "next month contains a high-severity regime-vulnerability shock sequence"
    if (
        candidate.get("high_severity_episode_count", 0) > 0
        and candidate.get("episode_construct_security_fragmentation_count", 0) > 0
    ):
        return "strong", "next month contains a high-severity fragmentation-linked deterioration sequence"
    if (
        candidate.get("high_severity_episode_count", 0) > 0
        and candidate.get("episode_construct_regime_vulnerability_count", 0) > 0
    ):
    ...
    if (
        ...
        return "plausible", "next month contains a fragmenting conflict-linked deterioration sequence"
    if (
        ...
        return "review", "next month contains broader acute stress but not yet a clear core deterioration episode"
    return "weak", "next month looks more like general stress than a clear acute political-risk deterioration signal"
...
def review() -> dict:
    ...
    for country in REVIEW_COUNTRIES:
        ...
        positives = [row for row in country_rows if int(row.get("acute_political_risk_next_1m", 0)) == 1]
        cases = []
        ...
        for row in positives:
            ...
            cases.append({
                ...
                "score_1m": row.get("acute_political_risk_signal_score_next_1m"),
                "label_1m": row.get("acute_political_risk_signal_label_next_1m"),
                "rating": rating,
                "note": note,
                "next_month_profile": {
                    "high_severity_episode_count": next_row.get("high_severity_episode_count", 0),
                    "fragmenting_episode_count": next_row.get("fragmenting_episode_count", 0),
                    "episode_construct_regime_vulnerability_count": next_row.get("episode_construct_regime_vulnerability_count", 0),
                    "episode_construct_security_fragmentation_count": next_row.get("episode_construct_security_fragmentation_count", 0),
                    "event_shock_flag": next_row.get("event_shock_flag", 0),
                    ...
                    "external_pressure_signal_present": next_row.get("external_pressure_signal_present", 0),
                    "economic_fragility_signal_present": next_row.get("economic_fragility_signal_present", 0),
                    "macro_stress_shift_flag": next_row.get("macro_stress_shift_flag", 0),
                },
            })
        countries.append({
            "country": country,
    ...
    return {
        ...
        "status": "private_internal_acute_political_risk_review",
        "source_file": str(PANEL.relative_to(ROOT)),
        "target_rule": payload.get("target_definitions", {}).get("acute_political_risk_next_1m", {}).get("rule_version"),
...
def main() -> None:
    ...
    print(f"Wrote acute political risk review to {OUT.relative_to(ROOT)}")
