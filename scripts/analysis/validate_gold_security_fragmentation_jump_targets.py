#!/usr/bin/env python3
"""
Validate the security-fragmentation-jump target layer against the stricter gold
subset.
"""
...
OUT = ROOT / "data" / "review" / "gold_security_fragmentation_jump_validation.json"
...
def main() -> None:
    ...
    for gold in gold_rows:
        ...
        if not panel_row:
            continue
        signal_score = int(panel_row.get("security_fragmentation_jump_signal_score_next_3m") or 0)
        signal_label = str(panel_row.get("security_fragmentation_jump_signal_label_next_3m") or "")
        ...
        matched.append({
            ...
            "predicted_label_3m": predicted,
            "signal_score_3m": signal_score,
            "signal_label_3m": signal_label,
    ...
    predicted_positive_rows = [
        ...
        if int(row.get("security_fragmentation_jump_signal_score_next_3m") or 0) >= 4
    ]
    ...
    output = {
        ...
        "status": "private_internal_gold_security_fragmentation_jump_validation",
        "panel_file": str(PANEL.relative_to(ROOT)),
        "gold_file": str(GOLD.relative_to(ROOT)),
        "target_rule": (
            panel_payload.get("target_definitions", {})
            .get("security_fragmentation_jump_next_3m", {})
            .get("rule_version")
    ...
    print(f"Wrote gold security-fragmentation-jump validation report to {OUT.relative_to(ROOT)}")
