#!/usr/bin/env python3
"""
Build a private adjudication queue for acute political-risk target review.

This stage turns the acute-political-risk review artifact into a cleaner
working queue so analysts can expand reviewed labels country by country
without replacing the broader proxy target layer.
"""
...
OUT = ROOT / "data" / "review" / "adjudication_queue_acute_political_risk.json"
...
def build() -> dict:
    ...
    for country_row in countries:
        ...
        for case in country_row.get("cases", []):
            ...
            if rating not in QUEUE_RATINGS:
                continue
            key = (country, str(case.get("trigger_month") or ""), "acute_political_risk_next_1m")
            if key in local_keys:
                continue
            rows.append({
                "country": country,
                "panel_date": str(case.get("trigger_month") or ""),
                "next_month": str(case.get("next_month") or ""),
                "target_name": "acute_political_risk_next_1m",
                "current_proxy_label": 1,
                "current_proxy_score_1m": case.get("score_1m"),
                "current_proxy_signal_label": case.get("label_1m"),
    ...
    return {
        ...
        "status": "private_internal_acute_political_risk_adjudication_queue",
        ...
        "description": (
            "Working queue of acute political-risk proxy positives that are not "
            "yet promoted into a reviewed adjudication layer."
        ),
...
def main() -> None:
    ...
    print(f"Wrote acute political-risk adjudication queue to {OUT.relative_to(ROOT)}")
