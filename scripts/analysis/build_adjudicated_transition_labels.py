#!/usr/bin/env python3
"""
Build a first private adjudicated irregular-transition label layer.

This stage promotes a narrow set of reviewed benchmark positives into tracked
labels so the country-month panel can start moving beyond pure proxy logic.
"""
...
OUT = ROOT / "data" / "modeling" / "adjudicated_irregular_transition_labels.json"
LOCAL_DECISIONS = ROOT / "data" / "review" / "adjudicated_transition_decisions.local.json"

APPROVED_COUNTRIES = {"Bolivia", "Chile", "Haiti", "Brazil"}
...
def build() -> dict:
    ...
    return {
        ...
        "status": "private_internal_adjudicated_label_artifact",
        ...
        "target_name": "irregular_transition_next_1m",
        "description": (
            "First narrow adjudicated irregular-transition label layer built "
            "from private benchmark review. This should remain selective and "
            "expand slowly."
        ),
...
def main() -> None:
    ...
    print(f"Wrote adjudicated labels to {OUT.relative_to(ROOT)}")
